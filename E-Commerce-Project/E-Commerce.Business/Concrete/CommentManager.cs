...
namespace E_Commerce.Business.Concrete
{
    public class CommentManager : ManagerBase,ICommentService
    {
        ...
        public async Task<IDataResult> GetAllByCustomerIdAsync(int customerId)
        {
            ...
            var comments = DbContext.Comments.Where(a => a.CustomerID == customerId);
            if (comments is null)
                return new DataResult(ResultStatus.Error, "Böyle bir yorum bulunamadı.");
            return new DataResult(ResultStatus.Success, comments);
        }
        ...
        public async Task<IDataResult> GetAllByProductIdAsync(int productId)
        {
            ...
            var comments = DbContext.Comments.Where(a => a.ProductID == productId);
            if (comments is null)
                return new DataResult(ResultStatus.Error, "Böyle bir yorum bulunamadı.");
            return new DataResult(ResultStatus.Success, comments);
        }
        ...
        public async Task<IDataResult> GetAllBySellerIdAsync(int sellerId)
        {
            ...
            var comments = DbContext.Comments.Where(a => a.SellerID == sellerId);
            if (comments is null)
                return new DataResult(ResultStatus.Error, "Böyle bir yorum bulunamadı.");
            return new DataResult(ResultStatus.Success, comments);
        }
